...
function autoscale(data) {
    ...
    var x_mid = d3.median(data, function (d) { return +d.weight; });
...
function autoscale_and_length(data, width, height) {
    ...
    // if (isNaN(x_min)) { x_min=0; }
    var x_max = d3.max(data.links, function (d) { return +d.weight; });
    // if (isNaN(x_max)) { x_max=600; }
    var x_mid = d3.median(data.links, function (d) { return +d.weight; });
    // if (isNaN(x_mid)) { x_mid=300; }
    var x_ave = d3.mean(data.links, function (d) { return +d.weight; });
    // if (isNaN(x_mid)) { x_ave=x_mid; }

    var exp = Math.LN2 / Math.log((1.002 * x_max - x_min) / (1.001 * x_mid - x_min));
    console.log('min, mid, max: ' + [x_min, x_mid, x_max] + ' gives exponent: ' + exp);
    pow_scale_d3 = d3.scale.pow().exponent(exp).domain([x_min, 1.001*x_max]).range([0.0, Math.pow(0.85, 0.25)]);
    function pow_scale(x) {
        return Math.min(Math.max(pow_scale_d3(x), 0), 2000);
    }


    var N_nodes = data.nodes.length;
    edge_length = Math.max(Math.min(50.0 * (width + height) * Math.pow(pow_scale(x_ave) / pow_scale(x_mid), 2) / Math.pow(N_edges, 0.75) / Math.pow(N_nodes, 0.25), 0.1), 50.0);

    console.log('scaled min, mid, ave, max: ' + [pow_scale(x_min), pow_scale(x_mid), pow_scale(x_ave), pow_scale(x_max)]);
    console.log("length: " + edge_length + " for " + N_edges + ", " +  N_nodes + " edges and nodes");
    return [pow_scale, edge_length];
}
...
function draw_force_directed_graph(graph, width, height, tag, process_group, process_name, friction, stiffness, charge, radius, opacity) {
    ...
    var edge_length = pow_scale_and_length[1];

    for(var indx in graph.nodes) {
        graph.nodes[indx].r = radius;
        graph.nodes[indx].group = typeof graph.nodes[indx].group !== 'undefined' ? +graph.nodes[indx].group : 1;
        graph.nodes[indx].x = typeof graph.nodes[indx].x !== 'undefined' ? +graph.nodes[indx].x : 20;
        graph.nodes[indx].y = typeof graph.nodes[indx].y !== 'undefined' ? +graph.nodes[indx].y : 17;
        }
    ...
    var force = d3.layout.force()
        ...
        .linkStrength(function (d) { return stiffness * Math.pow(pow_scale(d.weight), 3); } )
        .size([width, height]);
    ...
    var link = svg.selectAll(".link")
        ...
        .enter().append("line")
            .attr("class", "link")
            .style("stroke", "#888")
            .style("stroke-width", function(d) { return (stroke_width * Math.pow(pow_scale(d.weight), 4.0) + 0.25); })
            .style("opacity", function(d) { return (stroke_width * Math.pow(pow_scale(d.weight), 4.0) + 0.15); });
...
function graph_to_matrix(graph, directional, add_diagonal) {
    ...
    graph.nodes.forEach(function(node, i) {
        node.index = i;
        // accumulate weights from each edge (link) starting with a weight of 0
        node.weight = 0;
    ...
    graph.links.forEach(function(link) {
        ...
        graph.nodes[link.target].weight += link.weight;
        if (add_diagonal) matrix[link.target][link.target].z += link.weight;
        if (directional !== true) {
            matrix[link.target][link.source].z += link.weight;
            graph.nodes[link.source].weight += link.weight;
            if (add_diagonal) matrix[link.source][link.source].z += link.weight;
            }
        });
    return matrix;
    }
...
function draw_matrix_heat_map(graph, width, height, tag) { //, process_group, process_name, friction, stiffness, charge, radius, opacity) {
    ...
    var sort_choices = {
        ...
        weight: d3.range(matrix.N).sort(function(a, b) { return graph.nodes[b].weight - graph.nodes[a].weight; }),
    ...
    function order(weight) {
        x.domain(sort_choices[weight]);
